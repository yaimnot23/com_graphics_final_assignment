...
- **웹 게임 구동 데모 링크**: https://yaimnot23.github.io/com_graphics_final_assignment/src/index.html
- **기술 보고서(Repository) 링크**: https://github.com/yaimnot23/com_graphics_final_assignment/blob/main/docs/graphics_mapping_report.md
...
- **구현 코드**:
  ...
  - **코드 스니펫**:
    ...
    export function initBVH(enemyRoot) {
      ...
      if (!targetMesh) {
        console.warn("BVH: 적 메쉬를 찾을 수 없음");
        return;
      }
    ...
    export function updateGIProbes() {
      ...
      for (let p of probes) {
        if (spellIntensity > 0) {
          ...
          p.currentColor
            .copy(p.baseColor)
            .lerp(spellColor, giContribution * 1.5);
      ...
      if (spellIntensity > 0) {
        ...
        if (scene && scene.fog) {
          const fogColor = new THREE.Color(0x0a0910).lerp(
            spellColor,
            spellIntensity * 0.15,
          );
...
- **구현 코드**:
  ...
  - **코드 스니펫**:
    ...
    const eyeMat = new THREE.MeshBasicMaterial({
      color: this.isEnemy ? 0xff0044 : 0x00f2fe,
    });
