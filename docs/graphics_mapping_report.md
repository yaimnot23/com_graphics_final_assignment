...
* **게임 장르**: WebGL 기반 3D 로그라이크 덱빌딩 카드 게임
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![카드의 부채꼴 정렬 및 Quaternion SLERP 복구 모션](images/1_transform_slerp.png)
...
* **구현 코드**:
  ...
  * **코드 스니펫**:
    ...
    ```
* **📸 리포트용 캡쳐 가이드**:
  > 마우스 커서로 카드를 꾹 눌러 집어올린 채 **3D 씬 내에서 이리저리 드래그하며 조준 가이드 선이 발생해 보스 캐릭터를 겨냥하고 있는 화면**을 캡쳐하십시오. 스크린 공간의 마우스 터치 점이 3D 월드 투영 공간의 광선으로 변환되는 그래픽스 파이프라인 역산 원리를 증명하는 스크린샷으로 서술하십시오.

  ![마우스 드래그를 통한 3D 공간 레이캐스팅 및 타겟팅 조준선](images/2_raycaster.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![형광 녹색 MeshBVH 계층 박스 트리 구조 시각화](images/3_bvh.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![디버그 모드에서 노란색 Bounding Sphere 와이어프레임과 초록색 MeshBVH 간의 실시간 충돌 피격 장면](images/4_sphere_collision.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![PBR 재질 셰이딩 및 PCFSoftShadowMap 기반 부드러운 실시간 그림자 효과](images/5_phong_shadow.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![비등방성 필터링(Anisotropic Filtering 16배수) 적용에 따른 비스듬한 카드 표면 가독성](images/6_anisotropic.png)
...
* **구현 코드**:
  ...
  * **코드 스니펫**:
    ...
    ```
* **📸 리포트용 캡쳐 가이드**:
  > 게임 화면의 카메라를 약간 회전시키거나, 손패에 여러 카드가 얹혀 있는 상태에서 **카드 모서리 단면에 깃발처럼 입체적인 굴곡과 펄럭임(Wave)이 발생해 미세하게 구부러져 있는 측면 앵글**을 캡쳐하십시오. 이 펄럭임이 CPU 병렬 계산이 아닌 '정점 셰이더(Vertex Shader) 단의 Sin 함수 변형 연산 결과' 임을 셰이더 수식과 매핑하여 서술하십시오.

  ![커스텀 정점 셰이더(Vertex Shader)의 사인 파동 공식을 통한 마력 펄럭임 및 속도 비례 휨 변형 효과](images/7_vertex_shader.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![실린더형 스켈레탈 뼈대 계층 및 2/3단계 보스 관절 계층 상속 장식 장착(어깨 및 뿔) 상태](images/8_skinning_rigging.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![스펠 시전 전후 9개 프로브의 역제곱(Cosine Falloff) 거리 감쇄 및 Volumetric Fog 대기 간접광 발광 비교](images/9_gi_probes.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![타격 시 카메라 쉐이크/드라마틱 줌인 연동 및 속성색 상속 포물선 파티클 비산 효과](images/10_hit_particles.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![완전 방어 시 캐릭터 앞에 소환되는 초록빛 3D 쉴드 홀로그램(RingGeometry) 완전 차단 비교](images/11_shield_block.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![방어 카드 시전 시 캐릭터 주변에서 수직 상승하는 3D 상자(BoxGeometry) 파티클 장벽 효과](images/12_defense_vfx.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![전설 유물 '불꽃의 심장' 획득 전후 공격력 +5 가산 카드 Canvas 텍스처 실시간 갱신 및 툴팁 조회](images/13_relic_sync.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![투사체 비행 시 꼬리 방향으로 맺히는 Additive Blending 입자 잔상 궤적 효과](images/14_particle_trail.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![안개 대기 공간 속에서 삼각함수 파동 수식에 의해 천천히 부유하는 에테르 먼지 입자들](images/15_floating_dust.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![관절 행렬 변환 상속에 의해 머리 뼈 움직임을 자연스럽게 추종하는 3D 안광 결합 상태](images/16_glow_eyes.png)
...
* **📸 리포트용 캡쳐 가이드**:
  ...
  ![전장 경계에 세워진 고대 기둥과 자전 및 수직 조화 부유 운동을 수행하는 고대 크리스탈 데코](images/17_boundary_decors.png)
